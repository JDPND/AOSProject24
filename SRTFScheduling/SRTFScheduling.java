...
public class SRTFScheduling {
    public static void srtfScheduling(List<Processs> processses) {
        ...
        // Track transient events
        int transientEventTime = 5;  // For example, at time 5
        ...
        while (complete != n) {
            ...
            // Transient event occurs at the specified time
            if (currentTime == transientEventTime && !transientEventOccurred) {
                processses.get(shortest).remainingTime += 3; // For example, adding 3 units to the remaining time
                System.out.println("Transient event: Process P" + (processses.get(shortest).id + 1) + " remaining time adjusted by +3 units at time " + currentTime);
